...
```
                  ...
                  с непустыми cache-полями
                         ...
                         ▼
                  рендерит сегмент:
                    cache ↓122k +0.4k 1h:42m
                       │   │     │    │  └─ countdown до истечения TTL
                       │   │     │    └─ TTL bucket (1h cyan / 5m yellow)
                       │   │     └─ дописали в кеш
                       │   └─ прочитано из кеша
                       └─ префикс (dim)
```
...
- TTL bucket показывается явно: `1h` (dim cyan) для extended, `5m`
  (yellow) для короткого. Рядом — countdown `:Xm`/`:Xs` до истечения,
  цветовая шкала dim (>25% remaining) → yellow (10–25%) → orange
  (<10%) → red (`:0m`, истёк). Countdown оживает при
  `statusLine.refreshInterval` в settings.json — без него зависает
  на значении последнего event-driven refresh'а.
